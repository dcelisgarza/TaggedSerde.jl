...
[`to_json`](@ref) writes a value out; [`from_json`](@ref) reads it back. The element type of
...
[`from_json_direct`](@ref) instead builds each struct field-by-field with Julia's own `new`,
...
`to_json` is just [`encode`](@ref) followed by `JSON.json`. The `encode`/`decode` core has
...
Every exported function — [`to_json`](@ref), [`from_json`](@ref),
[`from_json_direct`](@ref), [`encode`](@ref), [`decode`](@ref) and [`SerdeContext`](@ref) —
is documented in full in the [Reference](@ref reference) section of the online
documentation.
